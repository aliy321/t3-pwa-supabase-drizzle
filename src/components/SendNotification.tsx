...
export default function SendNotification() {
	...
	> = async event => {
		...
		try {
			if (!process.env.NEXT_PUBLIC_WEB_PUSH_PUBLIC_KEY) {
				throw new Error(
					'Environment variables supplied not sufficient.'
				);
			}
			if (!registration) {
	...
	> = async event => {
		...
		try {
			...
			if (permission !== 'granted') {
				alert('Permission to send notifications is not granted');
				return;
			}
			...
			if (err instanceof Error) {
				if (err.name === 'TimeoutError') {
					console.error(
						'Timeout: It took too long to get the result.'
					);
					...
					console.error(
						'The AbortSignal.timeout() method is not supported.'
					);
				} else {
	...
	const testNotificationButtonOnClick: MouseEventHandler<
		HTMLButtonElement
	> = async event => {
		event.preventDefault();
		if ('serviceWorker' in navigator && navigator.serviceWorker.ready) {
			...
			alert('Service worker not ready or not supported.');
		}
	};
	...
	return (
		<div className="space-x-6">
			...
			<button type="button" onClick={testNotificationButtonOnClick}>
				Test Notification
			</button>
		</div>
	);
